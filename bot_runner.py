...
class TradingAgent:
    ...
    def ask_ai_analysis(self, symbol, price, rsi, adx, signal_type, trend_info=""):
        """🔄 ВОЗВРАЩЕН ПРЕДЫДУЩИЙ AI АНАЛИЗ"""
        ...
        for symbol, config in FUTURES_SYMBOLS.items():
            ...
            try:
                ...
                if ema9 > ema21 and 40 < rsi < 65 and adx > 20:
                    ...
                    trend_info = f"- Тренд: EMA9 ({ema9:.2f}) > EMA21 ({ema9:.2f}) - восходящий"
                
                # 📉 SHORT сигнал  
                elif ema9 < ema21 and 35 < rsi < 60 and adx > 20:
                    signal = "SELL"
                    signal_type = "SHORT_SIGNAL"
                    trend_info = f"- Тренд: EMA9 ({ema9:.2f}) < EMA21 ({ema9:.2f}) - нисходящий"
...
📊 Анализируем пар: {len(FUTURES_SYMBOLS)} фьюч., {len(SPOT_SYMBOLS)} спот
