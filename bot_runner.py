...
class TradingAgent:
    def __init__(self):
        ...
        # Копируем глобальные символы в атрибут класса
        ...
        print(f"📈 Торговых пар: {len(self.futures_symbols)} фьючерсов, {len(self.spot_symbols)} спотовых")
    ...
    def check_symbol_availability(self):
        ...
        try:
            ...
            # Проверяем каждый символ из начального списка
            initial_symbols = list(self.futures_symbols.keys())
            ...
            # Обновляем список доступных символов в атрибуте класса
            self.futures_symbols = {k: v for k, v in self.futures_symbols.items() 
                                   if k in available_futures}
            
            print(f"✅ Доступно {len(self.futures_symbols)} фьючерсных пар")
    ...
    def ask_ai_analysis(self, symbol, price, indicators):
        """Запрос анализа у AI"""
        ...
        Вердикт: YES для входа, NO для пропуска.
        """
        ...
        try:
            ...
            if order:
                self.positions[symbol] = {
                    'side': side,
                    'entry_price': order['price'] if order.get('price') else order.get('average', 0),
                    'amount': amount,
    ...
    def check_futures_signals(self):
        ...
        for symbol, config in self.futures_symbols.items():
            time.sleep(0.5)  # Уменьшенная пауза
...
💎 Активных пар: {len(self.futures_symbols)}
{'='*50}
"""
        print(status)
